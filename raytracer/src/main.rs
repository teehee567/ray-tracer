...
unsafe fn createShaderBuffers(device: &Device, data: &mut AppData) {
    data.shader_buffers.resize(1, value);
...
unsafe fn create_descriptor_sets(device: &Device, data: &mut AppData) -> Result<()> {
    ...
    for i in 0..data.swapchain_images.len() {
        ...
        let buffer_uniform_buffer_info = &[uniform_buffer_info];
        let ubo_write = vk::WriteDescriptorSet::builder()
            .dst_set(data.descriptor_sets[i])
            .dst_binding(0)
            .dst_array_element(0)
            .descriptor_type(vk::DescriptorType::UNIFORM_BUFFER)
            .buffer_info(buffer_uniform_buffer_info);

        let buffer_shader_buffer_info = &[shader_buffer_info];
        let sbo_write = vk::WriteDescriptorSet::builder()
            .dst_set(data.descriptor_sets[i])
            .dst_binding(1)
            .dst_array_element(0)
            .descriptor_type(vk::DescriptorType::STORAGE_BUFFER)
            .buffer_info(buffer_shader_buffer_info);
