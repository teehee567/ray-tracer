use vulkanalia::{prelude::v1_0::*, vk::{ImageSubresourceRange, ImageViewCreateInfo}};
use anyhow::Result;
...
pub unsafe fn create_image(instance: &Instance, device: &Device, data: &mut AppData) -> Result<()> {
    let image_info = vk::ImageCreateInfo::builder()
        ...
        .extent(vk::Extent3D {width: data.swapchain_extent.width, height: data.swapchain_extent.height, depth: 1})
        .mip_levels(1)
        .array_layers(1)
        ...
        let alloc_info = vk::MemoryAllocateInfo::builder()
            ...
            .memory_type_index(get_memory_type_index(instance, data, vk::MemoryPropertyFlags::DEVICE_LOCAL, mem_requirements)?);
...
pub unsafe fn transition_image_layout(device: &Device, data: &mut AppData) -> Result<()> {
    ...
    device.cmd_pipeline_barrier(command_buffer, vk::PipelineStageFlags::BOTTOM_OF_PIPE, vk::PipelineStageFlags::TOP_OF_PIPE, vk::DependencyFlags::empty(), &[] as &[vk::MemoryBarrier], &[] as &[vk::BufferMemoryBarrier], &[barrier]);
